...
import { formatDate, genDownloadUrl } from '@/libs/utils';
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      if (session && session?.user?.id) {
        const userDownloads = await getDownloadsByUserId(session?.user?.id);
                    ...
                    {downloads &&
                      ...
                      downloads?.map((download: any) => (
                        <tr key={download?.id}>
                            ...
                            <div className="ps-6 lg:ps-3 xl:ps-0 pe-6 py-3">
                              <div className="flex items-center gap-x-3">
                                <Image
                                  ...
                                  src={`/assets/product/images/${download?.product?.imageUrl}`}
                          ...
                          <td className="h-px w-px whitespace-nowrap">
                            <div className="px-6 py-1.5 text-center flex gap-4 items-center justify-end">
                              <Link href={`/downloads/${download.id}`}>
                                View
                              </Link>
                              <DownloadButton
                                dUrl={download?.product?.downloadLink}
                              />
                            </div>
                          </td>
                        </tr>
